const { getEthPrice, getSusdPrice } = require("./getprices");
const { readDb, writeDb } = require("./database");

const getDbData = async (token) => {
  let fromoutput = await readDb(token).catch((error) => {
    console.log(error);
  });
  let chartprice = [];
  let charttime = [];
...
const storeEthPrice = async () => {
  ...
  if (rawlastentry.length == 0) {
    let entry = 0;
    await writeDb(price, time, entry, token).catch((error) => {
      console.log(error);
    });
  } else if (rawlastentry.length > 0) {
    let lastentry = rawlastentry[rawlastentry.length - 1];
    await writeDb(price, time, lastentry, token).catch((error) => {
      console.log(error);
    });
  }
};

const storeSusdPrice = async () => {
  const token = "sc";
  let price = await getSusdPrice();
  const fetchtime = new Date();
  const time =
    fetchtime.getHours() +
    ":" +
...
module.exports = { getDbData, storeEthPrice, storeSusdPrice };
